...
process.on('message', function (msg, handle) {
    ...
    if (typeof json === 'string') {
        ...
        if (data.evt == "c_init") {
            ...
            NSLog.log('debug', 'out of hand. dismiss message', msg);
        }
    }
});
...
function warpJump(socket, goto, metadata, cb) {
    ...
    process.send(message, handle, function () {
...
function create() {
    var server = new FxConnection(8000,{runListen: false, glListener:false});
    var self = this;
    server.on('connection', function (client) {

        NSLog.log('info','Connection Clients name:%s (namespace %s)',client.name, client.namespace, process.argv[2]);
        if (process.argv[2] == "test2") {
            client.write(JSON.stringify({"action": process.argv[2]}));
        }
        var count = 0;
        var onDataHandle = function onDataHandle(data) {
            NSLog.log('debug','%s message :', process.argv[2], data, Buffer.byteLength(data));
            var json;
            try {
                json = JSON.parse(data);
                if (json.action == "echo") {
                    setInterval(function () {
                        client.write(JSON.stringify({"action": "echo2", count: ++count}));
                    },1000)
                } else if (json.action == "login") {
                    warpJump(client.socket, "test2", {}, function () {
                        setTimeout(function () {
                            process.exit()
                        }, 10000)
                    });
...
function run() {
    ...
    setTimeout(function (rss) {
        console.log((rss / 1000/ 1000) + "mb", ((process.memoryUsage()["rss"] - rss) / 1000/ 1000) + "mb");
        run();
    }, 1000, process.memoryUsage()["rss"])
