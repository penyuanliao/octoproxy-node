...
exports.editAssign = function (oAssign, obj, client) {
    const self = this;
    const path = self.AssignPath;
    const { assign2syntax, rules } = obj;
    try {
        ...
        for (var i = 0; i < conf["cluster"].length; i++) {
            ...
            if (conf["cluster"][i]["assign"] == oAssign) {
                ...
                if (typeof obj.options.tags == "string") {
                    conf["cluster"][i]["tags"] = obj["options"]["tags"].split(",");
                } else if (Array.isArray(obj.options.tags)) {
                    conf["cluster"][i]["tags"] = obj["options"]["tags"];
                } else if (obj.options.tags instanceof Set) {
                    conf["cluster"][i]["tags"] = [...obj["options"]["tags"]];
                }
                if (obj.options.env) conf["cluster"][i]["env"] = obj["options"]["env"];
                console.log(`writeFileSync ${path} ----->`, conf["cluster"][i]);
                break;
            }
        }
...
exports.setIPFilterAdd = function (obj, client) {
    ...
    try {
        ...
        if (!conf.deny) conf.deny = {};

        let { address, state, endTime, startTime, count, log, author } = obj;

        if (!conf.deny[address]) {
            conf.deny[address] = {
                enabled: state,
                startTime,
                endTime,
        ...
        NSLog.error('Configuration load file error:', e);
        if (typeof client != "undefined")
...
exports.setIPFilterDel = function (obj, client) {
    ...
    try {
        ...
        let data = fs.readFileSync(path);
        let conf = eval("("+data+")");
        let { address, author } = obj;

        if (!conf.deny) conf.deny = {};
        if (typeof conf.deny[address] != "undefined") {
            delete conf.deny[address];
        }
...
exports.getIPFilter = function () {
    return lib.getConfig(this.IPFilterPath);
};
