...
OctoManage.prototype.octoproxyRestart = function () {
    ...
    exec(command, function (error, stdout, stderr) {
        ...
        if (isNaN(state) == true) {
            ...
            exec("kill -9 "+ state);
            exec("pushd '../' sh " + Path.resolve(Path.dirname(__filename), "./startup.sh; popd;"), function (error, stdout, stderr) {
                console.log(error, Path.resolve(Path.dirname(__filename), "../startup.sh"));
            });
        }
    });
