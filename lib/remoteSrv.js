        ...
        client.manage.on("data", function (chunk) {
            ...
            for (var i = 0 ; i < arr.length; i++) {
                try {
                    ...
                    if (json["action"] == "clusterLockEnabled") self.manage.getClusterInfos();
                }
                catch (e) {
                    console.error(e);
                }
...
OctoManage.prototype._init = function () {
    ...
    sock.on('close', function () {
        ...
        setTimeout(self._connect.bind(self), 5000);
    });
...
/** 編輯指定名單規則 **/
OctoManage.prototype.editCluster = function (oAssign, nAssign) {
    var sock = this.socket;
    console.log(arguments);
    sock.write(JSON.stringify({"event":"editCluster","data":[oAssign, nAssign]}));
};
/** 動態刪除指定程序 - 伺服器重啟還會有 **/
