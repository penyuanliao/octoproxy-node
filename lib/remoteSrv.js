...
var createRemoteSrv = function () {
    ...
    s.insideLogPipe();
    var wssrv = s.outside();
    s.connectAdminCtrl(Admin_Ctrl_Port); //控制
    s.setupSysstat();
            ...
            for (var i = 0 ; i < arr.length; i++) {
                try {
                    ...
                    if (json["action"] == "clusterLockEnabled") {
                        ...
                        NSLog.log("info", "outside::getClusterInfos()");
...
remoteContrl.prototype.outside2 = function () {
    var self = this;

    var passive = this.oManage = new OctoManage();
    passive.runListening(Admin_Ctrl_Port);
    passive.on("message", function (json) {
...
remoteContrl.prototype.action = function (client, action) {
    ...
    switch (action["event"])
    {
        ...
        case "signout":
            ...
            break;
        case "setPods":
            client.write(JSON.stringify({"event":"setPods", "data": []}));
            break;
        case "getPods":
            client.write(JSON.stringify({"event":"getPods", "data": Object.keys(self.oManage.clients)}));
            break;
        case "getAssignConf2": {
...
remoteContrl.prototype.insideLogPipe = function () {
    ...
    var srv = this.insideSrv = net.createServer(function (socket) {
        ...
        function onDataHandler(chunk) {
            ...
            var n_keys = 0;
            if (typeof self.bindingSocks[socket.name] != "undefined") Object.keys(self.bindingSocks[socket.name]);
            if (typeof group == "undefined") {
                group = [];
            }
            ...
            var n;
            for (n = 0; n < n_keys.length; n++) {
                var nCli = self.bindingSocks[n_keys[n]];
                nCli.write(data)
            }
        ...
        for (var i = 0 ; i < arr.length; i++) {
            try {
                ...
                if (json["event"] == "getClusterInfos") {
                    ...
                    // self.setHistoryUse();
                    self.getPID();
                    console.log('admin.data', json.event);
...
remoteContrl.prototype.masterAdminCtrl = function () {
    var self = this;
    ...
    mgmtSrv.on('connection', function (client) {
        ...
        var onMessageHandle = function onMessageHandle(msg) {
            ...
            if (client.hasSetup == true || json.event == "setup") {
                ...
                if (json.event == "setup") {
                    ...
                    self.clients[json.data[0]] = new OctoManage(client);
                } else {
                    self.clients[client.tagDomain].masterHandle(msg);
                }
            }
        };
        ...
        var closeHandle = function closeHandle() {
            ...
            self.clients[client.tagDomain] = undefined;
            delete self.clients[client.tagDomain];
...
OctoManage.prototype.masterHandle = function (msg) {
    var self = this;
    var arr = msg.toString().match(/(\{.+?\})(?={|$)/g);
    ...
    for (var i = 0 ; i < arr.length; i++) {
        try {
            var json = JSON.parse(arr[i]);
            if (json["event"] == "getClusterInfos") {
                self.updatePodInfo(this.socket.tagDomain);
                self.recordLog(self.clusterInfos);

            } else if (json["event"] == "getAssign") {
                self.assignConfig = json["data"];
            } else if (json["event"] == "updatePodsCPUInfo") {
                self.updatePodsCPUInfo(json["data"]);
            } else {
                self.emit("handle", msg);
            }
        }
        catch (e) {
            console.error(e, msg.toString());
        }
    }
...
OctoManage.prototype.updatePodInfo = function (info) {
    ...
    while (i-- > 0) {
        ...
        if (typeof proc["memoryUsage"] != "undefined" && typeof proc["memoryUsage"]["rss"] != "undefined")
            memory[proc["name"]] = proc["memoryUsage"]["rss"]/1000 + "Kb";
        if (typeof this.podsCPUInfo[proc.pid] != "undefined" && typeof this.podsCPUInfo[proc.pid][1] != "undefined") {
            this.podsInfo[i]["cpuUsage"] = this.podsCPUInfo[proc.pid][1];
        } else {
...
OctoManage.prototype.updatePodsCPUInfo = function (info, tagDomain) {
    this.podsCPUInfo = info;
};
...
/** 列出子程序 **/
OctoManage.prototype.getAssign = function (tagDomain) {
    var sock = this.socket;

    if (typeof sock == "undefined") {
        if (typeof this.clients[tagDomain] == "undefined") return;
        sock = this.clients[tagDomain].socket;
    }
...
/** 顯示子程序資訊 **/
OctoManage.prototype.getClusterInfos = function (tagDomain) {
    var sock = this.socket;
    if (typeof sock == "undefined") {
        if (typeof this.clients[tagDomain] == "undefined") return;
        sock = this.clients[tagDomain].socket;
