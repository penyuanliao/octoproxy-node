...
const IsWorker          = (process.send instanceof Function);
const WPC_Logging_Port  = Config.WPC_Logging_Port || 10080;
const WPC_HTTP_Port     = Config.WPC_HTTP_Port || 10082;
const Admin_Ctrl_Port   = Config.managePort || 8100;
...
var createRemoteSrv = function () {
    ...
    ctrl.webServer = ctrl.outside();
    ctrl.httpServer = ctrl.setupHTTPServer();
    ctrl.BindingProcEvent({
        webServer:ctrl.webServer.app,
        httpServer:ctrl.httpServer
    });
    if (MASTER) {
        NSLog.log("info", "[Passive] Create listen <%s> for connections on a socket. isWorker: %s", Admin_Ctrl_Port, IsWorker);
        ...
        NSLog.log("info", "[Active] Create an endpoint for communication <%s:%s>. isWorker: %s", Server_IP, Admin_Ctrl_Port, IsWorker);
...
function RemoteControl() {
    ...
    if (Config.InfluxDBConf) {
        if (Config.InfluxDBConf.enabled) {
            this.syslog = this.setupInfluxDB({port:Config.InfluxDBConf.port, host:Config.InfluxDBConf.host});
        }
    }
}
...
RemoteControl.prototype.outside = function onConnect() {
    const self = this;
    NSLog.log("info", "starting create server.");
    var srv = new fxNet(WPC_HTTP_Port, { runListen: !IsWorker, glListener:false, baseVersion: "v2", zlibDeflatedEnabled: true });
...
RemoteControl.prototype.onManageMessage = function (client, data) {
    ...
    for (var i = 0 ; i < arr.length; i++) {
        ...
        try {
            ...
            else if (json.event == "onGetSchedule") {
    ...
    switch (action["event"])
    {
        ...
        case "getSysInfo": {
            if (MASTER == true) {
                ...
                this.getDiskUse();
                client.write(JSON.stringify({"event":"getSysInfo", "data": this.sysInfo, size: Buffer.allocUnsafe(75535).toString('hex')}));
        ...
        case "addCluster": {
            ...
            } else if (typeof action["data"][3] == "object")
            {
                ...
                if (options.cmd != "" && typeof options.cmd != "undefined" && options.cmd != null) {
                    ...
                    client.write(JSON.stringify({"event":"onAddCluster", data:false}));
                }
            }
        ...
        case "restartMultiCluster": {
            client.manage.restartMultiCluster(action["data"][0]);
            break;
        }
        case "refuseUser": {
        ...
        case "ipcMessage": {
            client.manage.ipcMessage(action["data"][0], action["data"][1], client);
            break;
        }
        case "getSchedule": {
            client.manage.getSchedule(client);
            break;
        }
        case "addSchedule": {
            client.manage.addSchedule(action["data"][0], action["data"][1], client);
            break;
        }
        case "cancelSchedule": {
            client.manage.cancelSchedule(action["data"][0], client);
            break;
        }
    ...
    var srv = this.insideSrv = net.createServer(function (socket) {
        ...
        socket.once('data', function (chunk) {
            ...
            if (socket.source == true) {
                ...
                NSLog.log("debug", 'Connect Logger Name:', socket.name);
            } else {
...
OctoManage.prototype.restartMultiCluster = function (group) {
    var sock = this.socket;
    sock.write(JSON.stringify({"event":"restartMultiCluster","data":[group]}));
};
/** 動態新增指定程序 - 伺服器重啟就無效 **/
...
OctoManage.prototype.signIn = function (user, pwd) {
    return (this.user == user && pwd == this.pwd);
};
...
OctoManage.prototype.getSchedule = function () {
    const sock = this.socket;
    if (sock.writable)
        sock.write(JSON.stringify({"event":"getSchedule","data":[]}));
};
OctoManage.prototype.addSchedule = function (params) {
    const sock = this.socket;
    if (sock.writable)
        sock.write(JSON.stringify({"event":"addSchedule","data":params}));
};
OctoManage.prototype.cancelSchedule = function (params) {
    const sock = this.socket;
    if (sock.writable)
        sock.write(JSON.stringify({"event":"cancelSchedule","data":params}));
};
...
RemoteControl.prototype.createHTTPConnected = function ({httpServer, handle, data}) {
...
RemoteControl.prototype.BindingProcEvent = function ({webServer, httpServer}) {
    /** process state **/
    ...
    process.on('message', (data, handle) => {
        ...
        if (typeof json === 'string') {
            ...
            if (data.mode === 'http') {
                ...
                NSLog.log('debug', "Conversion Socket.Hanlde from Child Process.", data);
                var socket = new net.Socket({
                    handle:handle,
                    allowHalfOpen:webServer.allowHalfOpen
                });
                socket.readable = socket.writable = true;
...
RemoteControl.prototype.setupHTTPServer = function () {
    ...
    var proxy = http.createServer(handleRequest);
