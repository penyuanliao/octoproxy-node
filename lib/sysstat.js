    ...
    exec(command,function (err, stdout, stderr) {
        ...
        if (stdout.search("/home") != -1) {
            for ( i = 0; i < drives.length; i++) {
                obj = drives[i].split(" ");
                if (obj[1] == '/home' || obj[1].indexOf('/home') != -1) {
...
sysstat.prototype.netDev = function (delay, callback) {
    ...
    if (type.toLowerCase() == 'linux') {
        ...
        var devPath = "./test/";
        if (process.cwd().indexOf("octoproxy-node/lib") != -1) devPath = "../test/";
        fs.readFile(devPath + "dev", onRead);
        setTimeout(function () {
            fs.readFile(devPath + "dev-2", onReadAfter);
        }, delay);
    }
};
...
sysstat.prototype.netSnmp = function (delay, callback) {
    ...
    if (type.toLowerCase() == 'linux') {
        ...
        var snmpPath = "./test/";
        if (process.cwd().indexOf("octoproxy-node/lib") != -1) snmpPath = "../test/";
        fs.readFile(snmpPath + "snmp", onRead);
        setTimeout(function () {
            fs.readFile(snmpPath + "snmp-1", onReadAfter);
        }, delay);
    }
...
sysstat.prototype.getNetDevices = function () {
    ...
    if (type.toLowerCase() == 'linux') {
        ...
        if (process.cwd().indexOf("octoproxy-node/lib") != -1) {
