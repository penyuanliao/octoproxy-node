...
mgmt.prototype.pushServer = function (port, host) {
    var self = this;
    var sock = this.pushSocket = new net.Socket();
    var client = sock;
        ...
        for (var i = 0 ; i < arr.length; i++) {
            ...
            switch (json.event){
                ...
                case "killClusterSync":
                    ...
                    break;
                case "assign-get":
                    sock.write(JSON.stringify(self.getAssign()));
    ...
    sock.on('close', function () {
        NSLog.log("info","Retry pushServer Client Connect.");
        setTimeout(function () {
...
mgmt.prototype.addCluster = function(file, name, mxoss, option, client) {
    ...
    var cluster = new daemon(file, cmdLine, {env:env, silent:false, execArgv:execArgv, pkgFile: pkg});
    cluster.init();
    cluster.name = name;
    cluster.mxoss = mxoss;
