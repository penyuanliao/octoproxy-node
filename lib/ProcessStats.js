        ...
        getUsage(function(startTime){
            setTimeout(function(){
                getUsage(function(endTime){
                    ...
                    if (cb) cb([new Date().getTime(),self.cpuPrecentage.toFixed(3)]);
                    self.emit('cpu.upgrade',[new Date().getTime(),self.cpuPrecentage.toFixed(3)]);
                    if (endTime == -1) clearTimeout(self.cpuUsageTick);
                });
            }, 1000);
            if (startTime == -1) clearTimeout(self.cpuUsageTick);
        });
