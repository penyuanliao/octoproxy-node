...
Server.prototype.httpConnection = function (req, res) {
    ...
    req.on("end", function onEnded() {
        ...
        let method = (segments[1] == "api") ? segments[1] + "/" + segments[2] : segments[1];//req.url.substr(1, req.url.length);
        let format;
        let data;
        let params = {};
        let json;
 ...
 * @param {String} [options.sockPath] ipc chancel path
 * @param {Boolean} [options.listen] listen socket port
 * @param {String} [options.delimiter] once data delimiter
 * @param {Boolean} [options.bound] 手動綁定net.socket
 * @param {Boolean} [options.tlsEnabled] 開啟傳輸層安全性協定
 * @param {Object} [options.tlsCerts] tls憑證
 * @param {String} [options.tlsCerts.cert] 憑證
 * @param {String} [options.tlsCerts.key] 私鑰
...
Client.prototype.connect = function (socket) {
    if (this.client && this.bound == true) {
        this.client.connect(socket);
        this.client.destroyed = false;
        this.client.isClosed = true;
        this.client.emit("connect");
    } else {
        NSLog.log("error", "Mode has Not bound.");
    }
}
