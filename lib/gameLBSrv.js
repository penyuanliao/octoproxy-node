...
function gameLBSrv(options) {
    ...
    this.glb_fork_d = new daemon(options.file,{silent:false,execArgv:["--nouse-idle-notification", "--max-old-space-size=" + options.moss]}); //心跳系統
...
gameLBSrv.prototype.init_daemon = function () {
    ...
    this.glb_fork_d.emitter.on('onSetGamePath', function (msg) {
        if (typeof self.getGamePathCB[msg.index] != "undefined" || self.getGamePathCB[msg.index] != null) {
            self.getGamePathCB[msg.index](msg["result"]);
            delete self.getGamePathCB[msg.index];
        }
    });
    function callback(d){
