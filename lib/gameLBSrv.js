 ...
 * @return {Promise}
 */
gameLBSrv.prototype.asyncGetLoadBalancePath = function (url_args, params) {
...
gameLBSrv.prototype.urlParse = function (params) {
    ...
    const dirname = dir;
    const slashOffset = (dirname[0] == xPath.sep ? 2 : 1);
    const offset = dirname.indexOf(xPath.sep, slashOffset);
    ...
    */
    if (folders[1] == 'api') {
        return "inind";
    } else if (folders[1] == 'endpoint') {
        let gp = dir.split("/") || [];
        let index = gp.indexOf(folders[1]);
        return {
            dir: gp.slice(index + 1).join("/")
        }
    } else if (folders[1] == 'web') {
        let gp = params.path.split("/") || ["", "", ""];
        return {
            dir: gp.slice(2).join("/")
        }
    } else if (params.specificBase && params.specificBase.has(folders[1])) {
        let gp = dir.split("/") || [];
        let index = gp.indexOf(folders[1]);
        return {
            dir: gp.slice(index + 1).join("/")
