...
class Dashboard extends events.EventEmitter {
    constructor(data) {
        ...
        setInterval(() => {
            NSLog.log("info", "Save Dashboard.json", this.octolog);
            Dashboard.writeFile(this.pathname, this.octolog);
        }, 10 * 1000 );
    }
    setup(data) {
        let date = new Date();
        ...
        this.octolog.incomeCount[date.getDay()] = {};
        this.count = 0;
    ...
    recordStatus(success) {
        ...
        if (!incomeCount[week]) incomeCount[week] = {};

        incomeCount[week][hour] = this.count;
        if (this.hour != hour) {
            this.count = 0;
            this.hour = hour;
        }
