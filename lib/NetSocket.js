        ...
        var onRuleData = function (data) {
            ...
            try {
                ...
                if (typeof tmp == "string" && json.action == "setup") {
                    ...
                    socket.write(JSON.stringify({action:"onSetup", event:true}) + socket.delimiter);
                    socket.emit("setup", json);
                } else {
                    socket.write(JSON.stringify({action:"onSetup", event:false}) + socket.delimiter);
...
NetSocket.prototype.init = function (options) {
    ...
    sock.on("data", function (data) {
        ...
        if (typeof self.configure != "undefined" && self.configure.buf === true) {
            ...
            while (index != -1) {
                try {
                    var bLen = index + self.delimiterLength;
                    var packet = sock.chunkBuffer.slice(0, index);
        ...
        var arr = sock.chunkBuffer.toString().match(self.delimiterRegexp);
        if (typeof arr == "undefined" || arr == null) return;
        for (var i = 0 ; i < arr.length; i++) {
            try {
                var one = arr[i];
                var len = Buffer.byteLength(arr[i]) + self.delimiterLength;
                ...
                NSLog.log("error","netSocket.onData error:%s", e, self.delimiterRegexp, json);
            }
        }
    });
...
NetSocket.prototype.send = function (message) {
    ...
    if ((this.socket && this.socket.writable && !this.socket.destroyed) != true) {
        console.log("error", "NetSocket.send has not writable")
        return;
    }
