...
try {
    ...
    if (os == "darwin") buffertools = require('./build/Release/Darwin/' + version + '.x/buffertools.node');
    else if (os == "linux") buffertools = require('./build/Release/Linux/' + version + '.x/buffertools.node');
	else buffertools = require('./build/Release/buffertools.node');
} catch (e) {
...
WritableBufferStream.prototype._append = function(buffer, encoding) {
	...
	if (this.buffer) {
		...
		this.buffer = Buffer.from(buffer.length);
