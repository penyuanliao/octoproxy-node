...
function amf0decNumber(buf) {
    ...
    //if (buf[1] == 0x00) return {len:2, value: null}; // todo null 時處理
    return { len: 9, value: buf.readDoubleBE(1) }
}
...
//    console.log('ODec',iBuf.readUInt8(0));
    while (iBuf.readUInt8(0) != 0x09) {
        ...
        if (prop.lost != -1) {
            iBuf = Buffer.concat([Buffer.from([prop.lost]), iBuf],iBuf.length+1);
        }
...
function amf0decUString(buf) {
    ...
    if (buf.readUInt8(0) == 0x06) {
        buf = Buffer.concat([Buffer.from([0x00]), buf],buf.length+1);
        lost = 0x00;
    }else if (buf.readUInt8(0) == 0x08 && buf.readUInt8(1) != 0x00 && buf.readUInt8(2) != 0x00 && buf.readUInt8(3) != 0x00)
    {
        buf = Buffer.concat([Buffer.from([0x00]), buf],buf.length+1);
        lost = 0x00;
    }
    var sLen = buf.readUInt16BE(0);
    return { len: 2 + sLen, value: buf.toString('utf8', 2, 2 + sLen), lost:lost }
}
...
function amf0decSArray(buf) {
    ...
    for (var count = buf.readUInt32BE(1); count; count--) {
        var slice_buf = buf.slice(len);

        var offset = 0;//&& slice_buf[offset] != 0x09
        //while ((slice_buf[offset] == 0) ) {
        //    offset++;
        //}

        ret = amf0DecodeOne(buf.slice(len + offset));
        a.push(ret.value);
        len += (ret.len + offset);
    }
...
function amfXDecodeOne(rules, buffer) {
    var _rules = rules[buffer.readUInt8(0)];
    if (!_rules) {
        console.log('Unknown field', buffer.readUInt8(0));
        throw new Error("Error: Unknown field #CODE 0x" + buffer[0].toString('16'));
    }
    return _rules(buffer);
}
...
function decodeAMF0Cmd(dbuf) {
    ...
    } else if (rtmpCmdDecode["call"]) {
        rtmpCmdDecode["call"].forEach(function (n) {
            if (buffer.length > 0) {
                var r = amf0DecodeOne(buffer);
                buffer = buffer.slice(r.len);
                resp[n] = r.value;
...
function encodeAdobeAMF0Cmd(opt) {
    var data = amf0EncodeOne(0x05);
    if (rtmpCmdDecode[opt.cmd]) {
        rtmpCmdDecode[opt.cmd].forEach(function (n) {
            if (opt.hasOwnProperty(n))
                data = Buffer.concat([data, amf0EncodeOne(opt[n])]);
...
module.exports = {
    ...
    encodeAdobeAmf0Cmd: encodeAdobeAMF0Cmd,
    amfType: amfType,
