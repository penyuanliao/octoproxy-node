...
/**
 *
 * read type 0x0a strict-array
 *
 * **/

var AMFStrictArray = function (data) {
    if (data){
        this.value = data;
    }
};
util.inherits(AMFStrictArray, AMFType);
defineConstant(AMFStrictArray.prototype, 'type', AMF.AMF_STRICT_ARRAY);
AMFStrictArray.prototype.__defineGetter__('byteLength', function () {
    var byteLength = 0;
    for (var k in this.value) {
        if (!this.value.hasOwnProperty(k)) continue;
        var type = normaliseType(this.value[k]);
...
AMFStrictArray.prototype.read = function(buf) {
    var a = [];
    var len = 5;
    var ret;
...
AMF.prototype.amfXDecodeOne = function (rules, buffer) {
